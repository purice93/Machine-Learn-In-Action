...
def smoSimple(dataMat,labelMat,C,toler,maxIter):
    dataM = mat(dataMat);labelM = mat(labelMat).transpose()
    b = 0
    m,n = shape(dataMat)
    ...
    while iter < maxIter:
        ...
        for i in range(m):
            ...
            if (labelM[i]*Ei < -toler and alphas[i] < C) or (labelM[i] > toler and alphas[i] > 0):
                j = selectJrand(i,m)
                Ej = float(multiply(alphas,labelM).T * (dataM * dataM[j,:].T)) - float(labelM[j])
                ...
                if L == H :
                    print("L == H")
                    continue
                eta = float(dataM[i,:]*dataM[i,:].T+dataM[j,:]*dataM[j,:].T-2.0 * dataM[i,:]*dataM[j,:].T)
                if eta <= 0:
                    print("eta <= 0")
                    continue
...
def smoSimple2(dataMatIn, classLabels, C, toler, maxIter):
